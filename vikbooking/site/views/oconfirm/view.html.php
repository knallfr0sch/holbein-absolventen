...
class VikbookingViewOconfirm extends JViewVikBooking
{
	public function display($tpl = null)
	{
		...
		if (count($rooms) != $proomsnum) {
			...
			$app->redirect(JRoute::_('index.php?option=com_vikbooking'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')));
			exit;
		}
			...
			if (!empty($coupon) && $valid_customer_coupon) {
				...
				if ($coupondateok === true) {
					...
					if ($couponroomok !== true) {
						$coupon = [];
						VikError::raiseWarning('', JText::_('VBCOUPONINVROOM'));
					}
					...
					$coupon = [];
					VikError::raiseWarning('', JText::_('VBCOUPONINVDATES'));
				}
			} else {
				if (!$valid_customer_coupon) {
					$coupon = [];
				}
				VikError::raiseWarning('', JText::_('VBCOUPONNOTFOUND'));
			}
		}
