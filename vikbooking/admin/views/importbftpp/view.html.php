...
{	
	...
	private function importThirdPartyReservations($tpp, $tpbookings, $tpp_rooms_map, $vbo_rooms_map, $tpp_bids)
	{
		...
		foreach ($tpp_bids as $tpbid => $val) {
			...
			// Booking History
			VikBooking::getBookingHistoryInstance()->setBid($newoid)->store('NB', JText::_('VBO_IMPBFTPP_BOOKHIST_DESCR') . " ({$tpbid})");
	...
	{
		JToolBarHelper::title(JText::_('VBOMAINIMPORTBFTPPTITLE'), 'vikbooking');
